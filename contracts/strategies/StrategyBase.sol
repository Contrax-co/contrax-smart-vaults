...
abstract contract StrategyBase is IStrategy {
  ...
  modifier onlyBenevolent() {
    require(harvesters[msg.sender] || msg.sender == governance || msg.sender == strategist, "Only Benevolent");
    _;
  }
  ...
  function setWithdrawalDevFundFee(uint256 _withdrawalDevFundFee) external onlyTimelock {
  ...
  function setPerformanceDevFee(uint256 _performanceDevFee) external onlyTimelock {
  ...
  function withdraw(address _asset) external onlyController returns (uint256 balance) {
  ...
  function withdraw(uint256 _amount) external onlyController {
    ...
    uint256 _feeDev = (_amount * withdrawalDevFundFee) / withdrawalDevFundMax;
    IERC20(want).safeTransfer(IController(controller).devfund(), _feeDev);

    uint256 _feeTreasury = (_amount * withdrawalTreasuryFee) / withdrawalTreasuryMax;
  ...
  function withdrawForSwap(uint256 _amount) external onlyController returns (uint256 balance) {
  ...
  function _distributePerformanceFeesBasedAmountAndDeposit(uint256 _amount) internal {
    ...
    if (_amount > 0) {
      ...
      // Performance fee
      IERC20(want).safeTransfer(IController(controller).devfund(), (_amount * performanceDevFee) / performanceDevMax);
