...
abstract contract ZapperBase is IZapper {
    ...
    function _returnAssets(
        ...
        for (uint256 i; i < tokens.length; i++) {
            ...
            if (balance > 0) {
                ...
                returnedAssets[i] = ReturnedAsset({
                    tokens: tokens[i],
                    amounts: balance
                });
            }
    ...
    function _zapIn(
        ...
        require(
            shares >= tokenAmountOutMin,
            "zapIn: Insufficient output vault shares"
        );
        ...
        returns (uint256 tokenOutAmount, ReturnedAsset[] memory returnedAssets)
    {
        ...
        require(
            tokenOutAmount >= desiredTokenOutMin,
            "zapOut: Insufficient output amount"
        );
    }
