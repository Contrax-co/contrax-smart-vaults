...
library OracleLibrary {
        ...
        returns (int24 arithmeticMeanTick, uint128 harmonicMeanLiquidity)
    {
        ...
        arithmeticMeanTick = int24(
            int256(tickCumulativesDelta) / int256(uint256(secondsAgo))
        );
        // Always round to negative infinity
        if (
            tickCumulativesDelta < 0 &&
            (int256(tickCumulativesDelta) % int256(uint256(secondsAgo)) != 0)
        ) arithmeticMeanTick--;
    ...
    function getBlockStartingTickAndLiquidity(
        ...
        tick = int24(
            int256(tickCumulative - prevTickCumulative) / int256(uint256(delta))
        );
